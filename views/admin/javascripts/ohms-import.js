...
(function ($) {
     ...
     * to be removed.
     */
    Omeka.OhmsImport.enableElementMapping = function () {
        $('form#ohmsimport .map-element').change(function () {
            var select = $(this);
     ...
     */
    Omeka.OhmsImport.confirm = function () {
        $('.ohms-undo-import').click(function () {
            return confirm("Undoing an import will delete all of its imported items. Are you sure you want to undo this import?");
        });
    };

    /**
     * Disable most options if Import from Ohms Report is checked
     */
    Omeka.OhmsImport.updateImportOptions = function () {
        ...
        if ($('#omeka_ohms_export').is(':checked')) {
          fields.slideUp();
        } else {
          fields.slideDown();
        }
