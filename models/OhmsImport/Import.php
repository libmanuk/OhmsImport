<?php
/**
 * OhmsImport_Import class - represents a ohms import event
 *
 * @copyright Copyright 2007-2012 Roy Rosenzweig Center for History and New Media
 * @license http://www.gnu.org/licenses/gpl-3.0.txt GNU GPLv3
 * @package OhmsImport
 */
class OhmsImport_Import extends Omeka_Record_AbstractRecord
{
    ...
    /**
     * Sets the column delimiter in the imported OHMS file
     *
     * @param string The column delimiter of the imported OHMS file
     */
    ...
    /**
     * Sets the file path of the imported OHMS file
     *
     * @param string The file path of the imported OHMS file
     */
    ...
    /**
     * Sets the original filename of the imported OHMS file
     *
     * @param string The original filename of the imported OHMS file
     */
     ...
     *
     * @param OhmsImport_ColumnMap_Set|array $maps The set of column maps
     * @throws InvalidArgumentException
     */
    public function setColumnMaps($maps)
    {
        if ($maps instanceof OhmsImport_ColumnMap_Set) {
            $mapSet = $maps;
        } else if (is_array($maps)) {
            $mapSet = new OhmsImport_ColumnMap_Set($maps);
        } else {
            throw new InvalidArgumentException("Maps must be either an "
                . "array or an instance of OhmsImport_ColumnMap_Set.");
        }
    ...
    /**
     * Imports the OHMS file.  This function can only be run once.
     * To import the same ohms file, you will have to
     * create another instance of OhmsImport_Import and run start
    ...
    /**
     * Returns the OhmsImport_File object for the import
     *
     * @return OhmsImport_File
     */
    public function getOhmsFile()
    {
        if (empty($this->_ohmsFile)) {
            $this->_ohmsFile = new OhmsImport_File($this->file_path, $this->delimiter);
        }
        return $this->_ohmsFile;
    }
     ...
     * @return OhmsImport_ColumnMap_Set The set of column maps for the import
     */
    public function getColumnMaps()
    {
        if ($this->_columnMaps === null) {
            $columnMaps = unserialize($this->serialized_column_maps);
            if (!($columnMaps instanceof OhmsImport_ColumnMap_Set)) {
                throw new UnexpectedValueException("Column maps must be "
                    . "an instance of OhmsImport_ColumnMap_Set. Instead, the "
    ...
    {
        $iit = $this->getTable('OhmsImport_ImportedItem');
     ...
     *
     * @param int $startAt A row number in the OHMS file.
     * @throws Exception
    ...
    protected function _importLoop($startAt = null)
    {
        try {
            ...
            $rows = $this->getOhmsFile()->getIterator();
            $rows->rewind();
            if ($startAt) {
                $rows->seek($startAt);
            }
            ...
            while ($rows->valid()) {
                ...
                $this->file_position = $this->getOhmsFile()->getIterator()->tell();
    ...
    protected function _undoImportLoop()
    {
        try {
            ...
            register_shutdown_function(array($this, 'stop'));
            $db = $this->getDb();
            $searchSql = "SELECT `item_id` FROM $db->OhmsImport_ImportedItem"
            ...
            while ($itemIds = $db->fetchCol($searchSql)) {
                ...
                foreach ($items as $item) {
                    ...
                    if ($batchSize > 0 && $deletedItemCount == $batchSize) {
                        ...
                        $db->delete($db->OhmsImport_ImportedItem, "`item_id` $inClause");
    ...
    /**
     * Adds a new item based on a row string in the OHMS file and returns it.
     *
     * @param string $row A row string in the OHMS file
    ...
    protected function _addItemFromRow($row)
    {
        ...
        $tags = $result[OhmsImport_ColumnMap::TYPE_TAG];
        $itemMetadata = array(
        ...
        // If this is coming from OHMS Report, bring in the itemmetadata coming from the report
        if (!is_null($result[OhmsImport_ColumnMap::TYPE_COLLECTION])) {
            $itemMetadata[Builder_Item::COLLECTION_ID] = $result[OhmsImport_ColumnMap::TYPE_COLLECTION];
        }
        if (!is_null($result[OhmsImport_ColumnMap::TYPE_PUBLIC])) {
            $itemMetadata[Builder_Item::IS_PUBLIC] = $result[OhmsImport_ColumnMap::TYPE_PUBLIC];
        }
        if (!is_null($result[OhmsImport_ColumnMap::TYPE_FEATURED])) {
            $itemMetadata[Builder_Item::IS_FEATURED] = $result[OhmsImport_ColumnMap::TYPE_FEATURED];
        }
        if (!empty($result[OhmsImport_ColumnMap::TYPE_ITEM_TYPE])) {
            $itemMetadata[Builder_Item::ITEM_TYPE_NAME] = $result[OhmsImport_ColumnMap::TYPE_ITEM_TYPE];
        }

        $elementTexts = $result[OhmsImport_ColumnMap::TYPE_ELEMENT];
        try {
        ...
        $fileUrls = $result[OhmsImport_ColumnMap::TYPE_FILE];
    ...
    {
        $ohmsImportedItem = new OhmsImport_ImportedItem();
        $ohmsImportedItem->setArray(array(
            'import_id' => $this->id,
            'item_id' => $itemId)
        );
        $ohmsImportedItem->save();
        $this->_importedCount++;
    }
